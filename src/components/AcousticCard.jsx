...
function DbMeter({ db }) {
  ...
  const pct   = Math.min(100, Math.max(0, db * 2.5))  // scale 0-40 dB → 0-100%
  const color = db < 3 ? '#34d399' : db < 12 ? '#6ee7b7' : db < 22 ? '#9ca3af' : db < 35 ? '#fbbf24' : '#f87171'
  return (
    <div style={{ margin: '0.75rem 0 0.25rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.3rem' }}>
        ...
        <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)', alignSelf: 'flex-end', paddingBottom: '0.25rem' }}>dB above baseline</span>
      </div>
...
export default function AcousticCard({ sensor }) {
  ...
  const isActive = status === 'listening' || status === 'calibrating'
